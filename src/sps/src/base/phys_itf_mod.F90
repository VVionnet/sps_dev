...
contains
   ...
   !*@/
      ...
      integer :: istat
      !---------------------------------------------------------------------
      call App_Log(APP_DEBUG,'[BEGIN] phys_config')
      F_istat = RMN_OK
      if (m_init_L) return
      m_init_L = .true.
      F_istat = RMN_ERR
      ...
      if (.not.RMN_IS_OK(istat)) then
         call App_Log(APP_WARNING,'(phys) Config file not found: '//trim(path_nml_S))
      else
         call App_Log(APP_INFO,'(phys) Reading Config: '//trim(path_nml_S)//'::'//trim(NML_S))
         F_istat = phy_nml(path_nml_S)
      endif

      if (F_istat /= PHY_OK) then
         call App_Log(APP_INFO,'(phys) Config reading problem')
      else
         m_isphy_L = .true.

      endif

      call App_Log(APP_DEBUG,'[END] phys_config')
      !---------------------------------------------------------------------
      return
   ...
   !*@/
      ...
      !---------------------------------------------------------------------
      call App_Log(APP_DEBUG,'[BEGIN] phys_init')
      F_istat = RMN_ERR
      ...
      if (.not.m_isphy_L) then
         call App_Log(APP_INFO,'(phys) Initialisation OK, Nothing done, no Physic')
         return
      endif
      ...
      if (RMN_IS_OK(err(1))) then
          if (p_nk < NK_MIN) then
              ...
              call App_Log(APP_DEBUG,'(phys_init) Number of levels ok')
          endif
      else
      ...
      if (RMN_IS_OK(istat) .and. G_ngrids > 1) then
          ...
          call App_Log(APP_DEBUG,'(phys_init) cannot wb_get ptopo/ngrids, G_ngrids will be set to 1')
      endif
      ...
      if (.not.RMN_IS_OK(err(4))) &
           call App_Log(APP_ERROR,'(phys_init) Problem in phy_init')
      
      
      !check input_type
      err(9) = wb_get('phy/input_type', input_type_S)
      if (WB_IS_OK(err(9))) then
         call App_Log(APP_DEBUG, 'phy/input_type: '//input_type_S)
         if (.not.(trim(input_type_S) == 'GEM_4.8')) then
            call App_Log(APP_ERROR, "input_type='GEM_4.8' required in namelist physics_cfgs")
            err(9) = RMN_ERR
         endif
      else
         call App_Log(APP_ERROR, '(phys_init) cannot wb_get phy/input_type')
         call App_Log(APP_ERROR, "input_type='GEM_4.8' required in namelist physics_cfgs")
         err(9) = RMN_ERR
      endif
      ...
      if (WB_IS_OK(err(10))) then
         if (.not. atm_external_L) then
            call App_Log(APP_ERROR, "fluvert='SURFACE' required in namelist physics_cfgs")
            err(10) = RMN_ERR
         else
            call App_Log(APP_DEBUG, "fluvert='SURFACE' detected namelist physics_cfgs: wb_get phy/atm_external=T")
         endif
      else
         call App_Log(APP_ERROR, '(phys_init) cannot wb_get phy/atm_external')
         call App_Log(APP_ERROR, "fluvert='SURFACE' required in namelist physics_cfgs")
      endif
      ...
      if (WB_IS_OK(err(11))) then
         if (.not.(sgo_tdfilter == -1.0) ) then
            call App_Log(APP_ERROR, 'sgo_tdfilter = -1.0 required in namelist physics_cfgs')
            err(11) = RMN_ERR
         endif
      else
         call App_Log(APP_ERROR, '(phys_init) cannot wb_get phy/sgo_tdfilter')
         call App_Log(APP_ERROR, 'sgo_tdfilter = -1.0 required in namelist physics_cfgs')
      endif
      ...
      if (RMN_IS_OK(F_istat)) then
         call App_Log(APP_INFO,'(phys) Initialisation OK')
      else
         call App_Log(APP_ERROR,'(phys) Problem in Initialisation')
      endif
      call App_Log(APP_DEBUG,'[END] phys_init')
      !---------------------------------------------------------------------
      return
   ...
   !*@/
      ...
      !---------------------------------------------------------------------
      call App_Log(APP_DEBUG,'[BEGIN] phys_input')
      F_istat = RMN_ERR
      ...
      if (.not.m_isphy_L) then
         call App_Log(APP_INFO,trim(app_msg)//' OK, Nothing done, no Physic')
         return
      endif
      ...
      if (.not.RMN_IS_OK(F_istat)) then
         call App_Log(APP_ERROR,trim(app_msg)//' End with Problems in Init')
         return
      endif
      ...
      call App_Log(APP_DEBUG,'[END] phys_input')
      !---------------------------------------------------------------------
      return
   ...
   !*@/
      !---------------------------------------------------------------------
      call App_Log(APP_DEBUG,'[BEGIN] phys_step')
      F_istat = RMN_ERR
      if (.not.m_init2_L) return
      ...
      call App_Log(APP_DEBUG,'[END] phys_step')
      !---------------------------------------------------------------------
      return
...
!!$      integer,parameter :: BUSPAR_NIK=2
      ...
      IF_INIT: if (.not.init_L) then
         ...
         if (.not.RMN_IS_OK(istat)) then
            call App_Log(APP_ERROR,'(phys_blocstats) Problem getting grid info')
            nstat = 0
         else
      ...
      VARLOOP: do ivar=1,nstat
         ...
         if (.not.RMN_IS_OK(istat)) then
            call App_Log(APP_TRIVIAL,'(phys_blocstats) Skipping var (not in bus): '//trim(stat_list_S(ivar))//')')
            cycle VARLOOP
         endif
         ...
         if (.not.RMN_IS_OK(istat)) then
            call App_Log(APP_TRIVIAL,'(phys_blocstats) Problem getting values from bus for: '//trim(stat_list_S(ivar)))
            cycle VARLOOP
         endif
...
!!$         k0 = max(1,kn-STAT_NK_MAX+1)
      ...
      call App_Log(APP_INFO,trim(app_msg)//' [End]   ------------')
      !---------------------------------------------------------------------
      return
   ...
   !*@/
      ...
      integer :: nlvls
      !---------------------------------------------------------------------
      call App_Log(APP_TRIVIAL,'(phys) Checking options consistency')
      ...
      if (.not.RMN_IS_OK(F_istat)) then
         call App_Log(APP_ERROR,'(phys_itf) Problem getting phy and levels cfgs to check options consistency')
         return
      endif
      ...
      if (use_zua_zta_L) then
         ...
         if (max_level > epsilon_4) then
            call App_Log(APP_ERROR,'(phys_itf) Incompatible sps cfg options: Can specify Lvl_list OR zua and zta, not BOTH')
            F_istat = RMN_ERR
         endif
         ...
         if (max_level < epsilon_4) then
            call App_Log(APP_ERROR,'(phys_itf) Incompatible cfg options: Must specify Lvl_list OR zua and zta')
            F_istat = RMN_ERR
         endif
   ...
   function priv_geom() result(my_istat)
      ...
      if (.not.(associated(dlat) .and. associated(dlon) .and. &
         ...
         call App_Log(APP_ERROR,'(phys_itf) geom - Problem getting gmm pointers')
         my_istat = RMN_ERR
         return
      endif
      ...
      if (.not.RMN_IS_OK(my_istat)) then
         call App_Log(APP_ERROR,'(dyn_input_init) Problem computing Lat-lon')
         return
      endif
      deg2rad = acos(-1.)/180.
      dlat = deg2rad*dlat
