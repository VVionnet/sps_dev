...
contains
   ...
   function dyn_press(my_readlist_S,my_nread,my_nk_max) result(my_istat)
      ...
      if (.not.(associated(ip1_m) .and. associated(ip1_t) .and. &
         ...
         call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(dyn_step) Probleme getting data to compute Pressure')
         return
      endif
      knm = ubound(ip1_m,1)
      knt = ubound(ip1_t,1)
      ...
      if (.not.RMN_IS_OK(istat)) then
         call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(dyn_step) Probleme getting pressure from vgrid')
         return
      endif
      ...
      my_istat = RMN_OK
      call Lib_Log(APP_LIBSPSDYN,APP_INFO,'(dyn_step) Compute Pressure [PW_PM:P, PW_PT:P]')
      !---------------------------------------------------------------------
      return
   ...
   function dyn_mfbr2mf_adapt(my_readlist_S,my_nread,my_nk_max,my_step) result(my_istat)
      ...
      if (.not.(associated(mf) .and. associated(mfbr) .and. &
           ...
           associated(p0))) then
         call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(dyn_step) Probleme getting data in dyn_mfbr2mf_adapt')
         my_istat = RMN_ERR
         return
      endif
      ...
      if (my_step == 0) then
         if (.not.any(my_readlist_S(1:my_nread) == 'mfbr')) then
            call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(dyn_step) Cannot run adaptation without MFBR')
            my_istat = RMN_ERR
            return
         endif
      endif
      ...
      if (nodiff_L) then
         call Lib_Log(APP_LIBSPSDYN,APP_INFO,'(dyn_step) Adaptation to High Res. Topography (nothing to do)')
         return
      endif
       ...
       call Lib_Log(APP_LIBSPSDYN,APP_INFO,'(dyn_step) Adaptation to High Res. Topography')
      !---------------------------------------------------------------------
      return
   ...
   function dyn_hr2hu(my_readlist_S,my_nread,my_nk_max) result(my_istat)
      ...
      if (.not.(RMN_IS_OK(my_istat) .and. &
         ...
         call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(dyn_step) Probleme Getting pointers, cannot compute HU')
         return
      endif
      lijk = lbound(hu)
      uijk = ubound(hu)
 ...
 !!$      k0 = lijk(3)
      ...
      call Lib_Log(APP_LIBSPSDYN,APP_INFO,'(dyn_step) Compute Specific Humidity HU')
      !---------------------------------------------------------------------
      return
...
!!$      my_istat = min(gmmx_data('MFBR',mf),my_istat)
      ...
      if (.not.(RMN_IS_OK(my_istat) &
         ...
         call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(dyn_step) Probleme Getting pointers, cannot compute GZ')
         return
      endif
      lijk = lbound(gz)
      uijk = ubound(gz)
...
!!$            gz(i,j,kn1) = gz(i,j,kn) + GRAV * RGASD/GRAV * tve * log(pm(i,j,kn)/pm(i,j,kn1))
      ...
      call Lib_Log(APP_LIBSPSDYN,APP_INFO,'(dyn_step) Compute AGL GZ ')
      !---------------------------------------------------------------------
      return
...
!!$      my_istat = min(gmmx_data('MFBR',mf),my_istat)
      ...
      if (.not.(RMN_IS_OK(my_istat) &
         ...
         call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(dyn_step) Probleme Getting pointers, cannot copy fields to PW')
         return
      endif
      ...
      call Lib_Log(APP_LIBSPSDYN,APP_INFO,'(dyn_step) Copy special ')
      !---------------------------------------------------------------------
      return
   ...
   subroutine priv_add_shuffle_list(my_readlist_S,my_nread,my_timeflags_S)
      ...
      do ivar = 1, nvars
         if (any(my_readlist_S(1:my_nread) == varlist2_S(2,ivar)) .and. &
            ...
            call Lib_Log(APP_LIBSPSDYN,APP_TRIVIAL,'(dyn_step) Add shuffled Var: '//trim(varlist2_S(1,ivar)))
         else
            call Lib_Log(APP_LIBSPSDYN,APP_TRIVIAL,'(dyn_step) Skipping shuffled Var: '//trim(varlist2_S(1,ivar)))
         endif
      enddo
      !---------------------------------------------------------------------
      return
   ...
   function priv_checklist(my_readlist_S,my_nread,my_step) result(my_istat)
      ...
      do ivar=1,nreq_vars
         if (my_nread == 0 .or. &
            ...
            my_istat = RMN_ERR
            call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(dyn_step) Missing mandatory var: '//trim(req_varlist_S(ivar)))
         endif
      enddo
      !---------------------------------------------------------------------
      return
