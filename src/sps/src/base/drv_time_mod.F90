...
contains
   ...
   function drv_time_config(F_cfg_basename_S) result(F_istat)
      ...
      !*@/
      !---------------------------------------------------------------------
      call Lib_Log(APP_LIBSPSDYN,APP_DEBUG,'[BEGIN] drv_time_config')
      F_istat = RMN_OK
      if (m_init_L) return
      F_istat = config_read(F_cfg_basename_S,'time_cfgs')
      call Lib_Log(APP_LIBSPSDYN,APP_DEBUG,'[END] drv_time_config')
      !---------------------------------------------------------------------
      return
   ...
   function drv_time_init(F_runstrt_S,F_dt_8,F_stepno,F_is_chkpt_L,F_is_last_L,F_ntime_levels,F_datev,F_swap_mode) result(F_istat)
      ...
      integer :: istat,datev
      !---------------------------------------------------------------------
      call Lib_Log(APP_LIBSPSDYN,APP_DEBUG,'[BEGIN] drv_time_init')
      istat = RMN_OK
      if (m_init_L) return
      m_init_L = .true.
      ...
      if (RMN_IS_OK(F_istat)) then
         call Lib_Log(APP_LIBSPSDYN,APP_INFO,'(drv_time) Initialisation OK')
      else
         m_init_L = .false.
         call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(drv_time) Problem in Initialisation')
      endif
      call Lib_Log(APP_LIBSPSDYN,APP_DEBUG,'[END] drv_time_init')
      !---------------------------------------------------------------------
      return
...
!!$      if (i3 < 0) error
   ...
   !*@/
      ...
      do ii = 1,nflds
         ...
         enddo
         if (nlvl == 2) then
            call Lib_Log(APP_LIBSPSDYN,APP_TRIVIAL,'(drv_time) shuffle: '//trim(namelist(1))//' <-> '//trim(namelist(2)))
         else if (nlvl == 3) then
            call Lib_Log(APP_LIBSPSDYN,APP_INFO,'(drv_time) shuffle: '//trim(namelist(1))//' <- '//trim(namelist(2))//' <- '//trim(namelist(3)))
         else
            call Lib_Log(APP_LIBSPSDYN,APP_TRIVIAL,'(drv_time) shuffle: '//trim(namelist(1))//' <-> ... <->'//trim(namelist(nlvl)))
         endif
      ...
      if (.not.RMN_IS_OK(istat)) then
         call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(drv_time) Problem doing shuffle')
      endif
      !---------------------------------------------------------------------
      return
   ...
   function priv_shuffle_copy(F_varlist_S) result(F_istat)
      ...
      do ivar = 1,size(F_varlist_S)-1
         ...
         if (F_istat /= 0) then
            call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(drv_time) Cannot copy fields, not same size/rank')
            return
         endif
         ...
         if (mymeta%l(3)%n == 0) then
            ...
            if (.not.(RMN_IS_OK(F_istat).and.associated(ptr2d1).and.associated(ptr2d2))) then
               call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(drv_time) Cannot copy fields, Problem getting data pointers')
               F_istat = RMN_ERR
               return
            endif
            if (any(lbound(ptr2d1) /= lbound(ptr2d2)) .or. &
                 any(ubound(ptr2d1) /= ubound(ptr2d2))) then
               call Lib_Log(APP_LIBSPSDYN,APP_ERROR,'(drv_time) Cannot copy fields, size mismatch')
               F_istat = RMN_ERR
               return
            endif
            ptr2d1(:,:) = ptr2d2(:,:)
